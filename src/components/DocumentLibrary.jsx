            ...
            {filtered.map(d => (
              <tr key={d.id}>
                ...
                <td style={{ display: 'flex', gap: 4 }}>
                  {d.file_path && getPolicyUrl(d.file_path) && <button className="btn btn-sm btn-primary" style={{ fontSize: 11, padding: '3px 10px' }} onClick={() => window.open(getPolicyUrl(d.file_path), '_blank')}>👁 View</button>}
